...
for i = 1:size(wall.xyz1,1)
    ...
    [wall.TE.refFac(i,:),wall.TE.transFac(i,:),wall.TM.refFac(i,:),wall.TM.transFac(i,:)] = ...
        FresnelCoefficients(1,wall.relativePerm(i,1),0:90,demoMode);
...
if losFlag == 1
    for i = 1:size(Tx.xyz,1)
        ...
        losBeamAngle.Tx.Ele(false(isnan(losBeamAngle.Tx.Ele(:,i))),i) = 0; % if nan turns the beam angle to 0
